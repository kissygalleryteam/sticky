 ...
 * @module sticky
 **/
KISSY.add('gallery/sticky/1.1/index',function (S, UA, Node, Base) {
	'use strict';
    ...
    S.extend(Sticky, Base, /** @lends Sticky.prototype*/{
        ...
        _supportAbsolute: function () {
            ...
            var el = this.get('el'),
            	top = this.get('top'),
	        	bottom = this.get('bottom'),
	        	target = this.get('target') || S.one('body'),             // IE 6下父级定位元素，默认为窗口
            ...
            if (this._stickyType == 'top') {
	            ...
	            if (distance <= top) {
	                ...
	                el.css({
	                    position: 'absolute',
	                    top: top + doc.scrollTop() - target.offset().top
	                });
	            ...
	            if (distance >= _top) {
	                ...
	                el.css({
	                    position: 'absolute',
	                    top: _top + doc.scrollTop() - target.offset().top
	                });
